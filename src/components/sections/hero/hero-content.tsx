...
const Typewriter = ({ words = [] }: { words: string[] }) => {
    ...
    useEffect(() => {
        ...
        // Tiempos optimizados para sensación premium
        let delay = 100;

        if (reverse) {
            delay = 25; // Borrado muy rápido (casi imperceptible)
        } else if (subIndex === words[index].length) {
            delay = 2500; // Pausa larga para leer (2.5s)
        } else {
            // Escritura natural con ligera variación
            delay = Math.random() * 30 + 50; // 50-80ms
        }
    ...
    return (
        <>
            {words.length > 0 ? `${words[index].substring(0, subIndex)}${blink ? "|" : " "}` : ""}
        </>
    );
};
...
const StatCard = ({ stat, index, delay }: { stat: any; index: number; delay: number }) => {
    ...
    return (
        <motion.div
            ...
            className="relative group rounded-xl p-4 transition-all duration-300 overflow-hidden"
            style={{
        ...
        >
            ...
            <div
                ...
                style={{
                    background: 'radial-gradient(circle at center, rgba(63, 136, 197, 0.15) 0%, transparent 70%)',
                }}
            />
            <div className="relative z-10">
                <div className="flex items-center justify-between mb-2">
                    ...
                    {stat.trend && (
                        <span className="text-xs font-semibold text-green-400 bg-green-400/15 px-2.5 py-1 rounded-full border border-green-400/20">
                            {stat.trend}
                        </span>
                    )}
                </div>
...
export default function HeroContent({ greetings, dict, lang }: HeroContentProps) {
    ...
    return (
        <div className="flex flex-col gap-6 z-10 w-full overflow-x-hidden px-4 sm:px-0">
            {/* Header */}
            <motion.div
            ...
            >
                <div className="min-h-[2rem] md:min-h-[2.5rem] mb-2 flex items-center justify-center">
                    <span className="text-xl md:text-2xl font-medium text-accent/90">
            ...
            <motion.div
                ...
                className="bg-white/10 backdrop-blur-md rounded-xl p-3 sm:p-6 w-full max-w-2xl mx-auto"
                style={{ border: '1px solid rgba(255, 255, 255, 0.1)' }}
            >
                <div className="w-full">
                    <h2 className="text-sm md:text-lg font-semibold text-white mb-3 text-left leading-relaxed">
                    ...
                    <form onSubmit={handleSearchSubmit} className="space-y-3">
                        ...
                        <div className="relative flex items-center">
                            ...
                            <Input
                                ...
                                className="w-full h-12 md:h-14 pl-12 pr-14 sm:pr-32 md:pr-36 rounded-full bg-white text-foreground placeholder:text-muted-foreground border-0 focus:ring-2 focus:ring-accent text-sm md:text-base transition-all"
                            />
                            <Button
                                type="submit"
                                size="lg"
                                className="absolute right-1.5 h-9 md:h-11 w-9 sm:w-auto sm:px-6 font-semibold bg-primary hover:bg-primary/90 text-white text-sm md:text-base rounded-full flex items-center justify-center overflow-visible"
                            >
                                {/* Icon only on mobile, text on sm+ */}
                                <svg className="block sm:hidden h-5 w-5 shrink-0" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24">
                                    <circle cx="11" cy="11" r="8" />
                                    <path d="m21 21-4.3-4.3" strokeLinecap="round" />
                                </svg>
                                <span className="hidden sm:block">{t.search_button || 'Buscar'}</span>
                            </Button>
                        </div>
                        {/* WhatsApp Subtle Link - Simpler on mobile */}
                        <div className="flex items-center justify-center gap-2 text-xs sm:text-sm text-gray-400">
                            <span className="hidden sm:inline">{t.search_disclaimer || 'Busca en nuestro inventario y en los principales portales de subastas de EE. UU.'}</span>
                            ...
                            <button
                                ...
                                className="inline-flex items-center gap-1.5 text-green-500 hover:text-green-400 transition-colors font-medium"
                            >
                                ...
                                <span className="sm:hidden">¿Dudas? Escríbenos</span>
