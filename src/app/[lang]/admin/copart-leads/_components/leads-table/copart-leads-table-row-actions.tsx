...
interface CopartLeadsTableRowActionsProps {
  user: UserProfile;
  onStatusChange: (user: UserProfile, status: 'active' | 'in-progress' | 'finished') => void;
  dict: any;
}

export function CopartLeadsTableRowActions({
  user,
  onStatusChange,
  dict
}: CopartLeadsTableRowActionsProps) {
  ...
  const getWhatsAppLink = () => {
    ...
    try {
        ...
        const message = encodeURIComponent(dict.admin_leads_page.whatsapp_message_copart.replace('{name}', user.names));
