...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    keywords: lang === 'es'
      ? ['contacto SUM Trading', 'WhatsApp importación autos', 'teléfono asesor subastas', 'cotización vehículos USA', 'consulta importación carros']
      : ['contact SUM Trading', 'auction car dealer Texas', 'salvage car consultants', 'vehicle import quote', 'car buyer assistance'],
    alternates: {
      ...
      <div className="container py-8 md:py-12 pt-28 md:pt-44">
                  ...
                  {content.process_timeline.steps.map((step: any, index: number) => {
                    ...
                    return (
                      <div key={index} className="relative">
                        <div className={`flex flex-col items-center text-center space-y-3 p-4 rounded-lg transition-all duration-300 hover:scale-105 hover:shadow-md ${isEven
                          ? 'bg-primary/5 hover:bg-primary/10'
                          : 'bg-accent/5 hover:bg-accent/10'
                          }`}>
                          <div className={`h-12 w-12 rounded-full flex items-center justify-center ${isEven
                          ...
                          <div>
                            <Badge variant="outline" className={`mb-2 text-xs ${isEven
                        ...
                        {index < 3 && (
                          <div className={`hidden md:block absolute top-10 -right-2 w-4 h-0.5 ${isEven ? 'bg-primary/30' : 'bg-accent/30'
                            }`} />
                        )}
                      </div>
                    );
        ...
        <StaggerContainer className="grid md:grid-cols-2 gap-8 md:gap-16 items-start max-w-6xl mx-auto">
          <FadeInItem className="order-2 md:order-1 space-y-6">
            ...
            {/* WhatsApp Button */}
            <Card className="bg-gradient-to-br from-green-50 to-emerald-50 border-green-200 transition-all duration-300 hover:shadow-lg hover:scale-[1.02]">
              <CardContent className="pt-6">
                <div className="flex flex-col sm:flex-row items-center gap-4 text-center sm:text-left">
                  ...
                  <div className="flex-1">
                    <h3 className="font-semibold text-base sm:text-lg mb-1 text-green-900">{content.whatsapp_button}</h3>
                  ...
                  <Button
                    asChild
                    className="bg-green-500 hover:bg-green-600 text-white transition-all duration-200 hover:scale-105 h-11 sm:h-12 text-sm sm:text-base px-5 sm:px-6 w-full sm:w-auto"
                  >
                    <a
