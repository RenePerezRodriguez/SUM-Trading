...
const ContactItem = ({ icon: Icon, title, value, href }: { icon: React.ElementType, title: string, value: string, href?: string }) => {
    ...
    return (
        <div className="flex items-start gap-3 sm:gap-4 group transition-all duration-300 hover:translate-x-2">
            <div className="flex h-10 w-10 sm:h-12 sm:w-12 items-center justify-center rounded-lg bg-primary/10 text-primary transition-all duration-300 group-hover:bg-primary group-hover:text-primary-foreground group-hover:scale-110 group-hover:rotate-3 shrink-0">
                <Icon className="h-5 w-5 sm:h-6 sm:w-6 transition-transform duration-300" />
            </div>
            <div className="min-w-0">
                <p className="font-semibold text-base sm:text-lg transition-colors duration-200 group-hover:text-primary">{title}</p>
                <Component
                    href={href}
                    target={href ? "_blank" : undefined}
                    rel={href ? "noopener noreferrer" : undefined}
                    className="text-muted-foreground text-sm sm:text-base break-words transition-colors duration-200 hover:text-primary"
                >
                    {value}
                </Component>
            </div>
...
export default function ContactInfo({ dict }: { dict: any }) {
    ...
    return (
        <div className="space-y-8">
            ...
            <div className="h-48 sm:h-64 w-full overflow-hidden rounded-lg transition-all duration-300 hover:shadow-lg" style={{ border: 'none' }}>
                <LogisticsMap />
            </div>
            <div className="mt-6">
